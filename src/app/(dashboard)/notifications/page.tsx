...
export default function NotificationsPage() {
  ...
  return (
    <PageShell
      ...
      action={<button type="button" onClick={markAllRead} className={btnSecondary}>Mark all read</button>}
    >
      {loading ? (
        <div className="space-y-3">{Array.from({ length: 5 }).map((_, i) => <div key={i} className="h-16 animate-pulse rounded-xl bg-slate-200 dark:bg-slate-800" />)}</div>
      ) : items.length === 0 ? (
        <EmptyState title="No notifications" description="You're all caught up!" />
      ) : (
        <div className="space-y-3">
          {items.map((n) => (
            <div key={n._id} className={`rounded-2xl border p-4 ${n.read ? "border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900" : "border-cyan-300 bg-cyan-50/50 dark:border-cyan-800 dark:bg-cyan-950/20"}`}>
              <div className="flex items-start justify-between gap-4">
                ...
                {!n.read && (
                  <button type="button" onClick={() => markRead(n._id)} className={btnPrimary}>Mark read</button>
                )}
              </div>
            </div>
          ))}
